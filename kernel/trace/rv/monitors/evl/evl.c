...
/*
 * EVL OOB wake-path RV monitor (composed automaton).
 *
 * Tracks per-CPU state of hard-IRQ-off, OOB scheduler, clock-tick, and
 * hardware IRQ handlers. Transition table generated from evl.dot, shipped
 * in include/generated/rv/evl_table.h.
 *
 * Per CPU: state (u8), timestamp (u64), edge stats, coverage bitmap.
 *
 * Hot path: next = transition[state][event]; account dwell; update state.
 *
 * Events: hard-IRQ-off (rv_irqoff.h), OOB scheduler/tick (tracepoints),
 * hardware IRQ (irq_handler_{entry,exit} filtered to IRQF_OOB).
 *
 * All helpers are notrace/no_instrument_function, run with hard IRQs off.
 */
...
/*
 * Per-IRQ handler dwell tracking. IRQs >= RV_EVL_HWIRQ_MAX fold to bucket 0.
 */
...
struct rv_evl_pcpu {
	 ...
	 *   blocker  = trap_kts - state_entered_at
	 *   dispatch = now - trap_kts
	 */
	u64			trap_kts;
	/* Inband-irqoff blocker if trap landed in IRQOFF_INBAND, else 0 */
	u64			blocker_ns;
	u64			unknown;
	/* Longest inband-irqoff blocker: dwell + caller IP */
	u64			longest_irqoff_ns;
	unsigned long		longest_irqoff_caller;
	unsigned long		irqoff_caller;
...
/* Forward-declared for rv_react(); defined at bottom */
static struct rv_monitor rv_evl_monitor;

/* Rate-limit reactor: 10 lines per 5s per CPU */
static DEFINE_RATELIMIT_STATE(rv_evl_unknown_rs, 5 * HZ, 10);

/*
 * ktime_get_mono_fast_ns() for cross-CPU stability and latmus correlation.
 */
...
/* True when FSM is in a hwirq handler state (nesting guard) */
static __always_inline notrace __attribute__((no_instrument_function))
